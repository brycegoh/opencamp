...
export const handleInbox = async (req: Request, res: Response) => {
  try {
    ...
    if (!user) {
      // In a federated system, we should create a local representation of the user
      // instead of returning a 404 error
      console.warn(`User ${username} not found for inbox request, creating local representation`);
      
      // For a complete implementation, we would fetch user info from the ActivityPub network
      // For now, we'll create a basic user with default values
